...
- EXP-0034 stress-tested `ALG-0030`: 20/20 cases passed across length-2, mixed-width, blocked-scope, and rare-count-two policy controls.
- EXP-0034 added `ALG-0031`, a rare-count-two support-guard ablation. It can filter count-two rare bodies under explicit validation but fails mixed one-valid/one-noisy rare-body controls.
- EXP-0035 added `ALG-0032`, a per-body rare inclusion validation selector. It passes 11/11 targeted cases, repairs mixed one-valid/one-noisy rare-body controls for count-one and configured count-two policies, and preserves unresolved/conflict/budget statuses for partial signal, validation conflicts, weak dominance, and too many rare bodies.
- EXP-0036 broadened `ALG-0032` to a split validation/final protocol. It passes 13/13 cases across singleton, length-2, mixed-width, count-two, cap-boundary, leakage, and blocked-scope controls, and shows `ALG-0029`/`ALG-0030` remain unresolved on mixed rare-body cases where `ALG-0032` selects. The cap-three case exposes a naive total of 7430 operations, making shared operation accounting the next blocker.
- EXP-0037 added an external shared operation-accounting report for `ALG-0029`, `ALG-0030`, and `ALG-0032`. Max reported totals drop from 1503 to 934 for `ALG-0029`, 962 to 638 for `ALG-0030`, and 7430 to 3167 for the `ALG-0032` cap-three case, but no selector status changed.
- EXP-0038 integrated the same shared totals into selector outputs and added report cross-checks. `ALG-0029`, `ALG-0030`, and `ALG-0032` still remain `smoke-tested` because accounting is not primitive-level, validation scope is explicit, and cap/upstream blocked-scope risks remain.
- EXP-0039 stress-tested `ALG-0032` rare-body caps: 11/11 cases passed, cap-plus-one refusals stayed cheap, but selected exhaustive enumeration exceeded the deep soft budget from two rare bodies onward.
- EXP-0040 added `ALG-0033`, a lower-cost direct-signal selector for per-body rare inclusion. It passes 16/16 smoke cases, preserves matched `ALG-0032` two-rare/count-two selections at lower cost, and keeps R=3/R=5 direct-signal scale cases under `B_deep`, but remains `smoke-tested` because non-unit interaction clauses are intentionally unresolved and split validation/final controls are not yet broad enough.
...
- `ALG-0029` is the body-inclusion validation selector for the `ALG-0025`/`ALG-0028` line; EXP-0033 completed split validation/final tests and EXP-0038 integrated shared-cost fields, but keep it at `smoke-tested` until primitive-level selector accounting and mixed rare-body / rare-count-two limits are addressed.
- `ALG-0030` is the body-count product selector over `ALG-0029` and `ALG-0026` alternatives; EXP-0034 improves width-robustness evidence and EXP-0038 integrates conservative shared product accounting, but keep it at `smoke-tested` until blocked upstream limits and primitive-level accounting are resolved.
- `ALG-0031` is the rare-count-two support-policy ablation of `ALG-0028`; keep it at `smoke-tested` because it is a prior, not a discovery claim, and group filtering fails mixed valid/noisy rare bodies.
- `ALG-0032` is the per-body rare inclusion validation selector over `ALG-0025`/`ALG-0028` loop-body evidence; EXP-0036 broadens split validation/final evidence, EXP-0038 removes the largest shared-discovery double count in selector outputs, and EXP-0039 confirms the exhaustive cap path exceeds budget from two rare bodies onward. Keep it at `smoke-tested` until primitive-level accounting, a lower-cost bounded selection rule, or narrower cap claims are available before broader non-toy validation.
- `ALG-0033` is the direct-signal lower-cost split from `ALG-0032`; EXP-0040 shows it is the current best path for operation-bounded per-body rare inclusion, but it needs split validation/final breadth and explicit validation-negative attribution checks before any promotion.
